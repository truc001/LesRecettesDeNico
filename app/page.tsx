...
export default function Home() {
  ...
  const [recipeLoadError, setRecipeLoadError] = useState(false);
  ...
  useEffect(() => {
    ...
    fetch("/api/recipes", { signal: controller.signal })
      ...
      .catch(() => { if (!controller.signal.aborted) setRecipeLoadError(true); })
      .finally(() => { if (!controller.signal.aborted) setLoadingRecipes(false); });
  ...
  return <>
    ...
    <header className="site-header"><a className="brand" href="#top" aria-label="Les recettes de Nico, accueil"><span className="brand-mark"><ChefHat size={25} aria-hidden="true" /></span><span>Les recettes <strong>de Nico<span className="brand-dot">.</span></strong></span></a><nav className="main-nav" aria-label="Navigation principale"><a href="#carnet" onClick={() => setFavoritesOnly(false)}>Le carnet</a><a href="#carnet" onClick={() => setFavoritesOnly(true)}><Heart size={16} aria-hidden="true" /> Mes favoris</a></nav>
      ...
      {isAdmin && <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) setEditingRecipe(null); }}><DialogTrigger asChild><Button className="add-button" onClick={() => openEditor()}><Plus size={18} /> Ajouter une recette</Button></DialogTrigger><DialogContent className="recipe-dialog"><DialogHeader><p className="form-kicker">{editingRecipe ? "Mise à jour" : "Ajout rapide"}</p><DialogTitle>{editingRecipe ? "Modifier la recette" : "Nouvelle recette"}</DialogTitle><DialogDescription>{editingRecipe ? "Modifiez uniquement ce dont vous avez besoin." : "Donnez-lui un nom pour l’enregistrer. Tout le reste peut être ajouté maintenant ou plus tard."}</DialogDescription></DialogHeader><form key={String(editingRecipe?.id ?? "new")} onSubmit={saveRecipe} className="recipe-form"><label className="recipe-title-field"><span>Nom de la recette <strong aria-hidden="true">*</strong></span><Input autoFocus required name="title" defaultValue={editingRecipe?.title} placeholder="Ex. Gratin de courgettes" autoComplete="off" /><small>Seul champ obligatoire</small></label><div className="recipe-main-fields"><label><span>Ingrédients <small>facultatif</small></span><Textarea name="ingredients" defaultValue={editingRecipe?.ingredients} placeholder={"3 courgettes\n20 cl de crème\n100 g de fromage râpé"} aria-describedby="ingredients-help" /><small id="ingredients-help">Un ingrédient par ligne</small></label><label><span>Préparation <small>facultatif</small></span><Textarea name="steps" defaultValue={editingRecipe?.steps} placeholder={"Couper les courgettes.\nAjouter la crème.\nFaire gratiner 25 minutes."} aria-describedby="steps-help" /><small id="steps-help">Une étape par ligne</small></label></div><details className="optional-details" open={Boolean(editingRecipe)}><summary><span>Ajouter des détails</span><small>Catégorie, temps, portions et résumé</small></summary><div className="optional-fields"><label>Catégorie<Input name="category" defaultValue={editingRecipe?.category} placeholder="Ex. Plat" list="recipe-categories" autoComplete="off" /><datalist id="recipe-categories">{categories.filter((category) => category !== "Toutes").map((category) => <option key={category} value={category} />)}</datalist></label><div className="form-grid"><label>Temps total<Input name="duration" defaultValue={editingRecipe?.duration === "À préciser" ? "" : editingRecipe?.duration} placeholder="Ex. 45 min" /></label><label>Portions<Input name="servings" defaultValue={editingRecipe?.servings === "À préciser" ? "" : editingRecipe?.servings} placeholder="Ex. 4 personnes" /></label></div><label>Petit résumé<Input name="description" defaultValue={editingRecipe?.description === "Une recette à essayer." ? "" : editingRecipe?.description} placeholder="Ce qui la rend spéciale" /></label></div></details><DialogFooter className="recipe-actions"><DialogClose asChild><Button type="button" variant="outline" disabled={saving}>Annuler</Button></DialogClose><Button type="submit" disabled={saving}>{saving ? "Enregistrement…" : editingRecipe ? "Enregistrer" : "Ajouter au carnet"}</Button></DialogFooter></form></DialogContent></Dialog>}</header>
    <main id="top"><section className="hero" aria-labelledby="hero-title"><div className="hero-text"><p className="eyebrow"><span /> Le bonheur est fait maison</p><h1 id="hero-title">Un peu de vous.<br />Beaucoup de <em>gourmandise.</em></h1><p className="hero-copy">Les recettes qu’on aime faire, refaire et partager. Bienvenue dans mon carnet de cuisine.</p><a className="primary-link" href="#carnet" onClick={() => setFavoritesOnly(false)}>Explorer les recettes <ArrowRight size={19} aria-hidden="true" /></a><div className="hero-note"><BookOpen size={19} aria-hidden="true" /><span>{loadingRecipes ? "Votre carnet de recettes, à votre rythme." : `${recipes.length} recettes à cuisiner, à votre rythme.`}</span></div></div><div className="hero-visual"><div className="hero-photo"><Image src="/tarte-tatin.png" alt="Tarte aux pommes caramélisées, tout juste servie à table" fill priority sizes="(max-width: 760px) 100vw, 50vw" /></div><div className="photo-note"><ChefHat size={25} aria-hidden="true" /><div><strong>Simplement bon.</strong><span>Le plaisir de cuisiner maison.</span></div></div><span className="photo-stamp" aria-hidden="true">Une pincée<br />de bonheur</span></div></section>
    <section className="cookbook" id="carnet" tabIndex={-1} aria-labelledby="collection-title"><div className="toolbar"><div><p className="section-kicker">À garder sous la main</p><h2 id="collection-title">{favoritesOnly ? "Mes favoris" : "Le carnet de recettes"}<span className="title-dot">.</span></h2><p className="section-description">Une envie, un ingrédient… et si on passait en cuisine ?</p></div><div className="search-box"><Search size={20} aria-hidden="true" /><label className="sr-only" htmlFor="recipe-search">Rechercher une recette ou un ingrédient</label><input id="recipe-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Une recette, un ingrédient…" />{query && <button aria-label="Effacer la recherche" onClick={() => setQuery("")}><X size={18} /></button>}</div></div>
      ...
      <p className="results-count" role="status" aria-live="polite" aria-atomic="true">{loadingRecipes ? "Chargement des recettes…" : recipeLoadError ? "Le carnet est momentanément indisponible." : `${filteredRecipes.length} recette${filteredRecipes.length !== 1 ? "s" : ""}${favoritesOnly ? " dans vos favoris" : " à découvrir"}${query ? ` pour « ${query} »` : ""}`}</p>{notice && <p className="notice" role="status">{notice}</p>}
      <div className="recipes-grid" aria-busy={loadingRecipes}>{recipeLoadError && <div className="empty-state"><h3>Impossible de charger les recettes.</h3><p>Votre carnet reste enregistré. Réessayez dans un instant.</p><button className="primary-link" onClick={() => window.location.reload()}>Réessayer</button></div>}{filteredRecipes.map((recipe) => <RecipeCard key={recipe.id} recipe={recipe} liked={favorites.includes(recipeKey(recipe))} onLike={() => toggleFavorite(recipe)} isAdmin={isAdmin} onEdit={openEditor} />)}{!loadingRecipes && !recipeLoadError && !filteredRecipes.length && <div className="empty-state"><Search size={30} aria-hidden="true" /><h3>{favoritesOnly && !query ? "Vos prochaines envies commencent ici." : "Pas encore de recette par ici."}</h3><p>{favoritesOnly ? "Ajoutez un cœur aux recettes qui vous font envie pour les retrouver ici." : "Essayez un autre ingrédient ou explorez toutes les catégories."}</p><button className="primary-link" onClick={() => { setQuery(""); setActiveCategory("Toutes"); setFavoritesOnly(false); }}>Voir toutes les recettes <ArrowRight size={18} aria-hidden="true" /></button></div>}{isAdmin && <button className="add-card" onClick={() => openEditor()}><span><Plus size={25} aria-hidden="true" /></span><strong>La prochaine bonne idée</strong><small>Ajouter une recette au carnet</small></button>}</div>
