...
export default function Home() {
  ...
  useEffect(() => { fetch("/api/auth/session").then((response) => response.json()).then((data: { isAdmin?: boolean }) => setIsAdmin(Boolean(data.isAdmin))).catch(() => undefined); }, []);
  useEffect(() => {
  ...
  async function logIn(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const password = String(new FormData(event.currentTarget).get("password") || ""); setLoggingIn(true); setLoginError("");
    try { const response = await fetch("/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setIsAdmin(true); setAuthOpen(false); setNotice("Mode administrateur activé."); } catch (error) { setLoginError(error instanceof Error ? error.message : "Connexion impossible."); } finally { setLoggingIn(false); }
  }
  async function logOut() { await fetch("/api/auth/login", { method: "DELETE" }); setIsAdmin(false); setNotice("Mode administrateur désactivé."); }
  ...
  return <main>
    <header className="site-header"><a className="brand" href="#top" aria-label="Les recettes de Nico, accueil"><span className="brand-mark"><ChefHat size={22} /></span><span>Les recettes<br /><em>de Nico</em></span></a><nav className="main-nav" aria-label="Navigation principale"><a className="nav-active" href="#carnet">Mon carnet</a><a href="#inspirations">Inspirations</a></nav>
      {isAdmin ? <div className="admin-controls"><span><LockKeyhole size={14} /> Administrateur</span><button onClick={logOut}>Quitter</button></div> : <Dialog open={authOpen} onOpenChange={setAuthOpen}><DialogTrigger asChild><button className="admin-login"><LockKeyhole size={15} /> Administration</button></DialogTrigger><DialogContent className="auth-dialog"><DialogHeader><DialogTitle>Administration</DialogTitle><DialogDescription>Seul l’administrateur peut ajouter ou modifier les recettes.</DialogDescription></DialogHeader><form onSubmit={logIn} className="recipe-form"><label>Mot de passe administrateur<Input required name="password" type="password" autoComplete="current-password" /></label>{loginError && <p className="form-error" role="alert">{loginError}</p>}<DialogFooter><Button type="submit" disabled={loggingIn}>{loggingIn ? "Connexion…" : "Se connecter"}</Button></DialogFooter></form></DialogContent></Dialog>}
